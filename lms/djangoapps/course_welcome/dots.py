...
def prepare_sections_with_grade(request, course):
    ...
    csm_scores = ScoresClient.create_for_locations(course.id, student.id, scorable_locations)
    ...
    for section in courseware_summary:
        ...
        for sub_section in section['sections']:
            earned += sub_section.graded_total.earned
            total += sub_section.graded_total.possible
    ...
    for section_index, chapter_info in final_chapters.items():
                        ...
                        if component.category == 'problem':
                            if component.graded:
                                ...
                                problem_score = get_score(
                                    ...
                                    csm_scores,
                                    None,
                                    course_structure[component.location]
                                )
                                unit_max_score += problem_score.possible
                                unit_score += problem_score.earned
        ...
        competency = None
        if int(section_points.get('total', 0)):
            competency = int(section_points.get('earned', 0)) == int(section_points.get('total', 0))
        section_context = {
            ...
            'points': {
                'total': int(section_points.get('total', 0)),
                'earned': int(section_points.get('earned', 0)),
